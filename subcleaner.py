...
Usage:
    subclean file.en.srt
    subclean file.en.ass --dry-run
    subclean /folder/of/subs -r
    subclean /folder/of/subs -r --report-only        # scan only, print report
    subclean movie.mkv --scan-video                  # probe embedded subs
    subclean /movies/ -r --scan-video                # scan a whole library
    subclean file.srt --gui                          # launch GUI pre-loaded
"""
import argparse
import sys
...
def cmd_scan_video(args) -> int:
    ...
    if total_ads > 0:
        ...
        print("      run subclean on the extracted file, then remux with mkvmerge.")
...
def main():
    parser = argparse.ArgumentParser(
        prog="subclean",
        description="Remove ads from subtitle files (.srt .ass .ssa .vtt)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  subclean movie.en.srt                    Clean a single file
  subclean movie.en.srt --dry-run          Detect only, do not write
  subclean /shows/ -r                      Recursive folder clean
  subclean /shows/ -r --report-only        Scan and report, do not clean
  subclean movie.mkv --scan-video          Probe embedded subtitle tracks
  subclean /movies/ -r --scan-video        Scan a whole library
  subclean movie.en.srt --gui              Launch GUI with file pre-loaded
""",
    )
